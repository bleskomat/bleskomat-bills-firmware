...
* `exchangeRate` - Hard-coded exchange rate to be appended to all signed withdraw LNURLs. Set to `0` to omit the exchange rate from signed LNURLs. When the exchange rate is present in the LNURL, the server will use this exchange rate instead of fetching the current rate.
