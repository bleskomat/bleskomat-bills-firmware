...
webSocketUri=wss://www.bleskomat.com/device
...
* `webSocketUri` - If the device is online and this value is non-empty, a websocket connection will be created to the provided URI. Example values:
	* `wss://www.bleskomat.com/device` -  Uses TLS encryption, provide the domain's certificate via `webCACert`
	* `ws://www.bleskomat.com/device` - Unencrypted websocket
* `webCACert` - The PEM-encoded TLS certificate for the web platform. This is required to create a secure WebSocket connection to the web platform. Fetch and print a certificate with the following command:
